...
model_col = next((c for c in df_inv.columns
                  if re.search(r'(model|material.*name)', c, re.I)), df_inv.columns[0])
df_inv.rename(columns={model_col:'Model'}, inplace=True)

qty_col = next((c for c in df_inv.columns if re.search(r'^qty', c, re.I)), None)
if not qty_col:
    sys.exit(f"❌ No quantity column found. Available: {df_inv.columns.tolist()}")
...
if not cost_col:
    sys.exit(f"❌ No total‑cost column found. Available: {df_inv.columns.tolist()}")
...
if not col_127:
    sys.exit(f"❌ 1.27 column missing. Available: {df_127.columns.tolist()}")
...
if not price_cols:
    sys.exit(f"❌ No list‑price column. Available: {df_master.columns.tolist()}")
df_master.rename(columns={price_cols[0]:'ListPrice'}, inplace=True)

# ──────────────────────────────────────
# 4. Ensure Model is string in all
# ──────────────────────────────────────
for df in (df_inv, df_127, df_master):

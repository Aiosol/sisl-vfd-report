...
• Reads exactly three CSVs in ./data/:
...
• Calculates COGS, COGS×1.75, List Price, 1.27 prices, 20/25/30 % discounts, GP %
• Sorts by capacity (0.4 K → 400 K) then by series D → E → F → A → HEL
• Generates an A4‑portrait PDF (0.6″ margins) in ./pdf_reports/
...
# 3) whatever’s left is the list‑price file
...
if not all((inv_csv, price127_csv, listprice_csv)):
    raise FileNotFoundError(f"Expected 3 CSVs in {DATA_DIR}; got {paths}")

...
– Sorts by Capacity (0.4→400 K) then Series (D,E,F,A,HEL)  
– Outputs an A4‑portrait PDF (0.6″ margins) named v0.5_YYYYMMDD.pdf  
"""
...
# 3. Detect & rename key columns in inventory
model_col = next((c for c in df_inv.columns if re.search(r'(model|material.*name)', c, re.I)), df_inv.columns[0])
df_inv.rename(columns={model_col: 'Model'}, inplace=True)

qty_col = next((c for c in df_inv.columns if re.search(r'^qty', c, re.I)), None)
if not qty_col:
    sys.exit(f"❌ No quantity column found in {INVENTORY_CSV}. Available: {df_inv.columns.tolist()}")
df_inv.rename(columns={qty_col: 'Qty'}, inplace=True)

cost_col = next((c for c in df_inv.columns if re.search(r'(total).*cost', c, re.I)), None)
if not cost_col:
    sys.exit(f"❌ No total‑cost column found in {INVENTORY_CSV}. Available: {df_inv.columns.tolist()}")
df_inv.rename(columns={cost_col: 'TotalCost'}, inplace=True)

# 4. Rename price‑127 columns
df_127.rename(columns={df_127.columns[0]: 'Model'}, inplace=True)
col_127 = next((c for c in df_127.columns if '1.27' in c), None)
if not col_127:
    sys.exit(f"❌ 1.27 column missing in {PRICE127_CSV}. Available: {df_127.columns.tolist()}")
df_127.rename(columns={col_127: 'Price127'}, inplace=True)

# 5. Rename master list‑price column
df_master.rename(columns={df_master.columns[0]: 'Model'}, inplace=True)
price_cols = [c for c in df_master.columns if re.search(r'price', c, re.I) and '1.27' not in c]
if not price_cols:
    sys.exit(f"❌ No list‑price column in {MASTER_CSV}. Available: {df_master.columns.tolist()}")
df_master.rename(columns={price_cols[0]: 'ListPrice'}, inplace=True)

# 6. Force numeric types (strip commas, convert)
def to_numeric_series(s):
    return pd.to_numeric(s.astype(str).str.replace(r'[^0-9.]', '', regex=True), errors='coerce')

df_inv['Qty']       = to_numeric_series(df_inv['Qty'])
df_inv['TotalCost'] = to_numeric_series(df_inv['TotalCost'])
df_127['Price127']  = to_numeric_series(df_127['Price127'])
df_master['ListPrice'] = to_numeric_series(df_master['ListPrice'])

# 7. Ensure Model is string everywhere
for df in (df_inv, df_127, df_master):
...
# 10. Sort keys
df['Capacity']    = df.Model.str.extract(r'-(\d+\.?\d*)K')[0].astype(float)
df['Series']      = df.Model.str.extract(r'FR-([A-Z]+)\d')[0]
...
output = os.path.join(PDF_DIR, f'SISL_VFD_Stock_Report_v{VERSION}_{today}.pdf')
...
pdf.cell(0, 0.3, f'SISL VFD Stock Report v{VERSION} – {datetime.now():%Y-%m-%d}', ln=1, align='C')
pdf.ln(0.2)

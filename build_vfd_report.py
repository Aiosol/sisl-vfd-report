...
• Filters out zero‑Qty lines, “FR‑S520SE‑0.2K‑19”, and any model containing “PEC”.
• Computes COGS, COGS×1.75, List Price, 1.27 price, 20 / 25 / 30 % discounts, GP %.
• Sorts by capacity (0.4 K → 400 K) then by series (D, E, F, A, HEL).
• Outputs an A4‑portrait PDF (0.6″ margins) to pdf_reports/ with name
  “SISL_VFD_Stock_Report_v0.5_YYYYMMDD.pdf”.
"""
...
if not cost_col:
    sys.exit(f"❌ No Total Cost column found. Columns: {df_inv.columns.tolist()}")
...
if not col_127:
    sys.exit(f"❌ 1.27 column missing. Columns: {df_127.columns.tolist()}")
df_127.rename(columns={col_127: "Price127"}, inplace=True)

# Master list price
df_master.rename(columns={df_master.columns[0]: "Model"}, inplace=True)
price_cols = [c for c in df_master.columns
              if re.search(r"price", c, re.I) and "1.27" not in c]
if not price_cols:
    sys.exit(f"❌ List‑price column missing. Columns: {df_master.columns.tolist()}")
...
df_inv["Qty"]        = to_num(df_inv["Qty"])
df_inv["TotalCost"]  = to_num(df_inv["TotalCost"])
df_127["Price127"]   = to_num(df_127["Price127"])
df_master["ListPrice"] = to_num(df_master["ListPrice"])

# ────────────────── 5. String‑ify model columns ───────────
for d in (df_inv, df_127, df_master):
...
series_order_map  = {"D": 0, "E": 1, "F": 2, "A": 3, "HEL": 4}
df["SeriesOrder"] = df["Series"].map(series_order_map).fillna(5)
df.sort_values(["Capacity", "SeriesOrder"], inplace=True)

# ────────────────── 9. PDF output ─────────────────────────
os.makedirs(PDF_DIR, exist_ok=True)
today   = datetime.now().strftime("%Y%m%d")
out_pdf = os.path.join(PDF_DIR,
...
# Header (ASCII hyphen avoids Unicode error)
